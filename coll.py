...
def iter_by_art(collect, doc_id):  #возвращение словарей с номером главы и статьи
    ...
    for i, num_chp in collect.itersplit(chp_sep_word):
        ...
        for j, num_art in new_col.itersplit(art_sep_word):
            num_art = num_art.split(' ')[1]
            d_art[(doc_id, num_chp, num_art)] = j
            d_rev[j] = (doc_id, num_chp, num_art)
    return d_art, d_rev
...
def iter_by_docs(docs, iter_by, it): #итерирование по документам, в параметрах по чему итерироваться(iter_by), и что именно нужно сделать при итерировании -- вернуть словарь(it=0) или постепенно каждую часть(it=1)
